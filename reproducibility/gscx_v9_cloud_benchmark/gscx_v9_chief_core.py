...
def simulate(controller, seed):
    ...
    for i in range(N_STEPS):
        ...
        if controller == "PID":
            ...
            risk = abs(A_pred) + 0.18 * max(0.0, d_ema) + 0.60 * persistence + 0.45 * memory
...
def main():
    ...
    df.to_csv("gscx_v9_chief_core_results.csv", index=False)
    ...
    summary.to_csv("gscx_v9_chief_core_summary.csv", index=False)
    ...
    for metric in ["survival_rate", "mean_peak", "mean_unsafe_time", "mean_energy", "mean_prod_per_energy"]:
        ...
        plt.tight_layout()
        plt.savefig(f"gscx_v9_{metric}.png", dpi=180)
        plt.close()

    print("\nSAVED:")
    print("  gscx_v9_chief_core_results.csv")
    print("  gscx_v9_chief_core_summary.csv")
    print("  gscx_v9_*.png")
